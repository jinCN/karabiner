...
function code(x){
  ...
  if(!names){
    ...
    let index
    let index2
    index = allName.indexOf('name:', index)
    while(index!==-1){
      index2=allName.indexOf('\n',index)
      if(index2===-1)index2=allName.length
      names.push(allName.substring(index+6,index2))
      index = allName.indexOf('name:', index+1)
    }
  }
  if(names.indexOf(x)!==-1)return x
  throw new Error(`invalid code ${x}`)
}
let names
let allName=`
//---
